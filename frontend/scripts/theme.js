...
slate.Variants = (function() {
    ...
    function Variants(options) {
        ...
        this.shadeItems = options.shadeItems;
        ...
        $(this.shadeItems, this.$container).on(
            'click',
            this._onShadeChange
        );
    }
    ...
    Variants.prototype = _.assignIn({}, Variants.prototype, {
        ...
        _onShadeChange: function(event) {
            var item = $(this).parent().next().find("option[value='"+ $(this).data('value') +"']");
            if(item) {
                $(item.parent())
                    .val($(this).data('value'))
                    .trigger('change');
        ...
        /** update shade */
        _updateShade: function(variant) {
            if ( variant.barcode && $(this.shadeItems).length ) {
                $(this.shadeItems).removeClass('selected');
                $(this.shadeItems+"[data-barcode='" + variant.barcode + "']").addClass('selected');
            }
        },
...
theme.Header = (function() {
    ...
    function init() {
        ...
        cache.$parents.on('mouseenter.siteNav', function() {
            var $el = $(this), $childImage = $el.find('.oc-child-image');
            if($childImage.length) {
                $childImage.attr('src', $childImage.data('default'));
            }
            if (!$el.hasClass(config.activeClass)) showDropdown($el);
        });

        //check when we're leaving a dropdown and close the active dropdown
        cache.$parents.on('mouseleave.siteNav', function() {
            hideDropdown(cache.$activeDropdown);
        });

        cache.$subMenuLinks.on('mouseenter', function() {
            var $el = $(this), image = $el.data('image'), $parent = $el.parents('.site-nav__childlist-item');
            $parent.siblings('.site-nav__childlist-item').find('img').attr('src', image);
        });

        // close dropdowns when on top level nav
        cache.$topLevel.on('mouseenter.siteNav', function() {
            if (cache.$activeDropdown.length) {
                hideDropdown(cache.$activeDropdown);
            }
        });

        cache.$subMenuLinks.on('mouseleave.siteNav', function(evt) {
    ...
    function hideDropdown($el) {
        ...
        $(selectors.body).off('mouseenter.siteNav');
    ...
    function positionFullWidthDropdowns() {
        ...
        $listWithCenteredDropdown.each(function() {
            ...
            var fullWidthDropdownOffset = $hasCenteredDropdown.position().top + 45;
...
//   	var dropdwn =  document.getElementsByClassName("thatlink")
//     var x = document.getElementsByClassName("thatlink").addEventListener("mouseover", function(){
// 	    dropdwn.classList.add(".site-nav__dropdown-on");
//     }
...
theme.Filters = (function() {
    ...
    Filters.prototype = _.assignIn({}, Filters.prototype, {
        ...
        _initDescriptionShorter: function () {
            this.$expandableButton = this.$expandableButton || $('.expandable-text__button');

            this.$expandableButton.click(function () {
                this.$expandableButton.closest('.collection-resizable-description').toggleClass('show-more');
...
theme.Product = (function() {
    function Product(container) {
        ...
        this.selectors = {
            ...
            cartButton: '.site-header__cart',
            cartCount: '[data-cart-count]',
            cartCountBubble: '[data-cart-count-bubble]',
            cartPopup: '[data-cart-popup]',
            cartPopupItem: '.cart-popup-item',
            cartPopupItemList: '.cart-popup-item-list',
            ...
            cartPopupActions: '.cart-popup__actions',
            cartPopupEmpty: '.cart-popup__empty',
            cartPopupLoading: '.cart-popup__loading',
            miniCartRemove: '[data-mini-cart-remove]',
            miniCartItem: '[data-mini-cart-item]',
            miniCartNote: '[data-mini-cart-note]',
            loader: '[data-loader]',
            ...
            productQuantityButtons: '.product-quantity__buttons',
        ...
        this.attributes = {
            miniCartItemIndex: 'data-mini-cart-item',
            dataCartItemQuantity: 'data-cart-item-quantity',
            dataCartQuantityAction: 'data-cart-quantity-action',
            dataMiniCartItemHandle: 'data-mini-cart-item-handle'
        };
    ...
    Product.prototype = _.assignIn({}, Product.prototype, {
        ...
        _initCartButton: function() {
            var $cartButton = $(this.selectors.cartButton);
            ...
            $(document).on(
                'click',
                this.selectors.miniCartRemove,
                this._onMiniCartRemoveItem.bind(this)
            );

            $(document).on(
                'click',
                `${this.selectors.productQuantityButtons} > button`,
                this._onMiniCartQuantityItemUpdate.bind(this)
            );

            $(document).on(
                'change',
                this.selectors.miniCartNote,
                this._onMiniCartNoteChange.bind(this)
            );
            ...
            $(document).on(
                ...
                function (event) {
                    ...
                    if($target[0] === $(this.selectors.cartPopupWrapper)[0]) {
        ...
        _initVariants: function() {
            var options = {
                ...
                shadeItems: this.selectors.shadeItems,
            ...
            $(this.selectors.productForm, this.$container).on(
                ...
                function(evt) {
                    ...
                    if (!isInvalidQuantity && this.ajaxEnabled) {
                        ...
                        var productId = +$data.find('[name="id"]').val();
                        ...
                        $.getJSON('/cart.js').then(function(cart) {
                            ...
                            var product = cart.items.filter(function(item){
                                return item.id === productId;
                            })[0];

//                             if (product && product.quantity >= 2) {
//                                 this._showErrorMessage('Only 2 of the same product is allowed');
...
setTimeout(function(){
 ...
 jQuery.getJSON('/cart.js', function(cart) { 
             //console.log(cart, 'pppppppppppppppppppppppppp'); 
			} );
              },1800)
...
/*------- */
        ...
        _addItemToCart: function(data, comparePro) {
            let sendData = comparePro ? data : $(data).serialize();
            var params = {
                url: '/cart/add.js',
                data: `${sendData}`,
                dataType: 'json'
            ...
            $.post(params)
                .done(
                    function(item) {
                        ...
                        $(".cart-popup__scrollable-content").load(location.href + " .cart-popup__scrollable-content");
                    }.bind(this)
                )
                .fail(
                    function(response) {
...
window.dataLayer.push({
  event: 'Add to Cart',
        ...
        _showMiniCartLoading: function() {
            $(this.selectors.cartPopupLoading).fadeIn();
        },


        _hideMiniCartLoading: function(){
            setTimeout(function() {
                $(this.selectors.cartPopupLoading).fadeOut();
            }.bind(this), 1000);
        },
        ...
        _setupCartPopup: function() {
            ...
            this.$cartPopupItemList = this.cartPopupItemList || $(this.selectors.cartPopupItemList);
            ...
            this.$cartPopupActions = this.$cartPopupActions || $(this.selectors.cartPopupActions);
            this.$cartPopupEmpty = this.$cartPopupEmpty || $(this.selectors.cartPopupEmpty);

            this._setupCartPopupEventListeners();
            this._updateCartPopupContent();
        },

        _updateCartPopupContent: function() {
             $.getJSON(window.root_url + '/cart.js').then(
                function(cart) {
                    this.$cartPopupItemList.html('');

                    if (!cart.items.length) {
                        this.$cartPopupActions.addClass(this.classes.hidden);
                        this.$cartPopupEmpty.removeClass(this.classes.hidden);
                    } else {
                        this.$cartPopupActions.removeClass(this.classes.hidden);
                        this.$cartPopupEmpty.addClass(this.classes.hidden);
                    }

                    $.each(cart.items, function(index, item){
                        var image = this._setCartPopupImage(item.featured_image.url, item.featured_image.alt).outerHTML;
                        var description = this._setCartPopupProductDetails(
                            item.product_has_only_default_variant,
                            item.options_with_values,
                            item.properties
                        );
					 var up_price = parseFloat(item.line_price / 100);
                     up_price = up_price.toFixed(2);
                     /*
                        this.$cartPopupItemList.append(`
                            <div class="cart-popup-item">
                              <div class="cart-popup-item__image-wrapper" data-cart-popup-image-wrapper>
                                <div class="cart-popup-item__image cart-popup-item__image--placeholder" data-cart-popup-image-placeholder>
                                  <div data-placeholder-size>${image}</div>
                              ...
                              <div class="cart-popup-item__description">
                                <div>
								  ...
								  <div class="cart-product-price">${up_price}</div>
                                  <ul class="product-details" aria-label="{{ 'cart.popup.product_details' | t }}" data-cart-popup-product-details>
                                   ${description}
                                   <li>
                                      <p class="cart__remove" data-mini-cart-item="${index+1}">
                                        <a href="/cart/change?line=${index+1}&amp;quantity=0" class="text-link text-link--accent" data-mini-cart-remove><span>` + window.theme.strings.remove + `</span></a>
                                      </p>
                                    </li>
                                  </ul>
                                </div>
                                <div class="cart-popup-item__quantity">
                                  ...
                                  <span aria-hidden="true"><span>` + window.theme.strings.qty + `</span>: <span>${item.quantity}</span></span>
                                  <span aria-hidden="true" data-cart-popup-quantity></span>
                                  <div class="product-quantity__buttons" data-cart-item-quantity="${item.quantity}"  data-mini-cart-item="${index+1}" data-mini-cart-item-handle="${item.handle}">
                                    <button aria-label="Decrease quantity ${item.product_title}" aria-disabled="true" data-cart-quantity-action="decrease" ${+item.quantity === 1 ? 'disabled' : ''}>
                                    ...
                                    <button aria-label="Increase quantity ${item.product_title}" aria-disabled="false" data-cart-quantity-action="increase" ${+item.quantity >= 2 ? 'disabled' : ''}>
        ...
        _onMiniCartNoteChange: function (evt) {
            var note = evt.currentTarget.value;
            ...
            $.post(params).fail(
                function () {
                    console.error('Unable to add the gift message.');
                }.bind(this)
            );
        },

        _onMiniCartRemoveItem: function(evt) {
            evt.preventDefault();
            var $remove = $(evt.target);
            if($remove.hasClass('disabled')) return;

            var $lineItem = $remove.closest(this.selectors.miniCartItem);
            var index = $lineItem.attr(this.attributes.miniCartItemIndex);

            $remove.addClass('disabled');
            $remove.text('Removing...');

            this._updateMiniCartItemQuantity({ quantity: 0, line: index });
        },

        _onMiniCartQuantityItemUpdate(evt) {
            evt.stopPropagation();
            this._showMiniCartLoading();

            var $action = $(evt.currentTarget);
            var $item   = $action.parent();
            var action  = $action.attr(this.attributes.dataCartQuantityAction);
            var quantity     = +$item.attr(this.attributes.dataCartItemQuantity);
            var index   = +$item.attr(this.attributes.miniCartItemIndex);
            var itemHandle   = $item.attr(this.attributes.dataMiniCartItemHandle);
            var self    = this;
            ...
            this._shouldUpdateCartItemQuantity(itemHandle, action, quantity)
                .then(function () {
                        self._updateMiniCartItemQuantity({ quantity: quantity, line: index });
                    }
                ).finally(this._hideMiniCartLoading.bind(this));
        },
        ...
        _setCartPopupImage: function(imageUrl, imageAlt) {
            ...
            image.src = theme.Images.getSizedImageUrl(imageUrl, '200x');
            image.alt = imageAlt;
        ...
        _getVariantOptionList: function(variantOptions) {
         ...
         _getPropertyList: function(properties) {
            ...
            propertiesArray.forEach(function(property) {
              ...
              if(property[0] == 'Text'){
              	mono_text = property[1]; 
              }
              if(property[0] == 'Color'){
              	mono_color = property[1]; 
              }
              if(property[0] == 'Text' || property[0] == 'Color'){
              }else{
                // Line item properties prefixed with an underscore are not to be displayed
                if (property[0].charAt(0) === '_') return;
          	...
          	if(mono_text !=''){
              if(mono_color=='Blind'){
              mono_color ="rgba(255, 255, 255, 0.2)";
              }
				propertyListHTML +=	"<li class='cart_popup_mono_text'><span style='color:"+mono_color+"' color='"+mono_color+"'>"+mono_text+"</span> Classic Monogram Patches Added</li>";
          	}
        ...
        _hideCartPopup: function(event) {
            ...
            $('body').off('click').removeClass('unscrollable');
        },
        ...
        _updateMiniCartItemQuantity: function(data) {
            var params = {
                url: '/cart/change.js',
                data: data,
                dataType: 'json'
            ...
            $.post(params).always(this._setupCartPopup.bind(this));
        },

        _shouldUpdateCartItemQuantity: function(itemHandle, action, quantity) {
            function onlyAllow2Products(resolve, reject) {
                if (quantity >= 3 || quantity < 1) reject();

                resolve();
            }

            function onlyAllow2ProductsExcludingThoseTagsAndType() {
                // $.getJSON(`/products/${itemHandle}.js`, function(product) {
                //     if (
                //         product.type === 'Homme' ||
                //         product.tags.includes('Prêt-à-porter')
                //     ) {
                //         resolve();
                //     }
                //
            ...
            return new Promise(onlyAllow2Products)
        },
...
$(document).ready(function() {
    ...
    var productSection = new theme.Product();
    productSection._initCartButton();

    window.compareBuyNow = async function compareBuyNow(variantId, quantity) {
      console.log("working atc")
      let data = `id=${variantId}&quantity=${quantity}`;
      await productSection._addItemToCart(data, true);
    }
...
/* Code of create an account of t&c and privacy policy  **/

  $(document).ready(function() {
    $('body').on('click', '[name="condition"]', function() {
      if ($('#RegisterForm-Newsletters').is(':checked')) {
        $(this).submit();
      }
      else {
        alert("You must agree with the terms & conditions & privacy cookie policy.");
        return false;
      }
    });
  });
...
var userinput = $("#ContactFooter-email").val();
var pattern = /^\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b$/i
if(pattern.test(userinput)) {
  $("#Subscribe").prop("disabled",false);
} else {
...
/* End Code of account of t&c and privacy policy  **/


jQuery(document).ready(function(){
  var setint = '';
  jQuery('.site-nav--has-dropdown.site-nav--has-centered-dropdown').hover(function(){
	var _self = this;
	var count = 0;
	jQuery('.site-nav--has-dropdown.site-nav--has-centered-dropdown').removeClass("site-nav--active-dropdown");
    clearInterval(setint);
    setint = setInterval(function(){
		//console.log(count);
		jQuery('.site-nav--has-dropdown.site-nav--has-centered-dropdown').removeClass("site-nav--active-dropdown");
    	jQuery(_self).addClass('site-nav--active-dropdown');
      	if(count++>100){
       		clearInterval(setint); 
     	}
    },1);
  ...
  jQuery("#RegisterForm-Newsletters").on("change",function(){
 	console.log(jQuery(this));
    if(jQuery("#RegisterForm-Newsletters").prop("checked")) {
...
jQuery(document).ready(function(){
  ...
  //jQuery('.shopify-payment-button__button').text('QUICK BUY');
  setTimeout(function(){
  jQuery('.shopify-payment-button__button').text('QUICK BUY');
  },1000);
//   jQuery('.site-nav--has-dropdown.site-nav--has-centered-dropdown a').hover(function(){
//     jQuery('this').parent('.site-nav--has-dropdown.site-nav--has-centered-dropdown').addClass('site-nav--active-dropdown');
//   });
  
  
  $(".tabbing-section").on("click",function() {
    if($(this).hasClass("active")) {
      $(".tabbing-section").removeClass("active");
      $(".product-single__description").slideUp("fast");
    } else {
      $(".tabbing-section").removeClass("active");
      $(".product-single__description").slideUp("fast");
      $(this).addClass("active").next(".product-single__description").slideDown("slow");    
    }
  });
